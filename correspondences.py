...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9, 
    ...
    print("find_correspondences() image1_batch.size", image1_batch.size())
...
def find_correspondences_tensor(image1: torch.Tensor, image2: torch.Tensor, num_pairs: int = 10, load_size: int = 224, layer: int = 9, 
                         salency_layer: int = 11, num_heads: int = 6, register: bool = False,
                         facet: str = 'key', bin: bool = True, thresh: float = 0.05, model_type: str = 'dinov2_vits14',
                         stride: int = 7, return_patches_x_y: bool = True) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]],
                                                                              Image.Image, Image.Image]:
    ...
    resize = transforms.Resize(load_size)
    image1 = resize(image1)
    image2 = resize(image2)

    descriptors1 = extractor.extract_descriptors(image1.to(device), layer, facet, bin, has_register=register)
    num_patches1, load_size1 = extractor.num_patches, extractor.load_size
    print("num_patches1", num_patches1)
    print("descriptors1", descriptors1.shape)
    descriptors2 = extractor.extract_descriptors(image2.to(device), layer, facet, bin, has_register=register)
    num_patches2, load_size2 = extractor.num_patches, extractor.load_size
    print("descriptors2", descriptors2.shape)

    # extracting saliency maps for each image
    saliency_map1 = extractor.extract_saliency_maps(image1.to(device), salency_layer, num_heads, register)[0]
    saliency_map2 = extractor.extract_saliency_maps(image2.to(device), salency_layer, num_heads, register)[0]

    n_clusters = 0
    while(n_clusters < num_pairs):
        print(f"tresh: {thresh}")
        # threshold saliency maps to get fg / bg masks
        fg_mask1 = saliency_map1 > thresh
        fg_mask2 = saliency_map2 > thresh
    ...
    for y1, x1, y2, x2 in zip(img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show):
        ...
        points2.append((y2_show, x2_show))
    if return_patches_x_y:
        return points1, points2, [img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show], descriptors1, descriptors2, num_patches1
